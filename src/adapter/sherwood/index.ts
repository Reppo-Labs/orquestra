...
export function createSherwoodAdapter(deps: SherwoodDeps = {}): DatanetAdapter {
  ...
  // Single upstream (one network's pool list) — one throttle slot; the instance
  // persists across cycles, so the last success time lives in its closure.
  ...
  return {
    ...
    async discover(ctx: AdapterContext): Promise<CandidatePod[]> {
      ...
      if (lastFetchAt !== undefined && t - lastFetchAt < minInterval && lastPools.length > 0) {
        ...
        // A fetch failure (GeckoTerminal 429, network blip) means no candidates
        // THIS cycle — it must not throw into runCycle and mark the datanet errored.
        try {
        ...
        lastFetchAt = t // only successful fetches arm the throttle
        lastPools = pools
      }
