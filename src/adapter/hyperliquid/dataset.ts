...
export function buildHlDataset(wallet: string, rawFills: unknown, datanetId: string): CandidatePod | null {
  ...
  const wins = closed.filter((f) => fin(f.closedPnl) > 0).length
  const sumPnl = closed.reduce((a, f) => a + fin(f.closedPnl), 0)
  let peak = 0, cum = 0, maxDd = 0
  for (const f of closed) { cum += fin(f.closedPnl); peak = Math.max(peak, cum); maxDd = Math.max(maxDd, peak - cum) }
  ...
  const trades = closed.map((f) => ({
    ...
    size: fin(f.sz), fill_price: fin(f.px),
    outcome: { pnl: Math.round(fin(f.closedPnl) * 100) / 100, win: fin(f.closedPnl) > 0 },
