...
async function readUint(rpcUrl: string, to: string, sel: string): Promise<number> {
  ...
  // rpcFetch: RPC_URL may list fallback endpoints (src/reppo/rpcEndpoints.ts).
  const res = await rpcFetch(fetch, rpcUrl, { method: 'POST', headers: { 'content-type': 'application/json' }, body })
