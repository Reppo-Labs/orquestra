...
describe('checkReppoVersion', () => {
  it('passes silently when the CLI meets the required version', async () => {
    const warn: string[] = []
    const ok = await checkReppoVersion({ getVersion: async () => '0.12.3', warn: (m) => warn.push(m) })
    expect(ok).toBe(true)
    expect(warn).toEqual([])
  })

  it('passes for a newer version (numeric, not lexicographic)', async () => {
    expect(await checkReppoVersion({ getVersion: async () => '0.13.0', warn: () => {} })).toBe(true) // 0.13 > 0.12.3
    expect(await checkReppoVersion({ getVersion: async () => '0.100.0', warn: () => {} })).toBe(true) // 0.100 > 0.12.3 numerically; lexically would fail
  ...
  it('parses the version despite a date/build token in PREFIX, ADJACENT, or SUFFIX position', async () => {
    expect(await checkReppoVersion({ getVersion: async () => '@reppo/cli 0.12.3', warn: () => {} })).toBe(true)
    expect(await checkReppoVersion({ getVersion: async () => 'built 2024.01 reppo v0.12.3', warn: () => {} })).toBe(true)
    // date IMMEDIATELY AFTER the product name (no v between) — year token filtered
    expect(await checkReppoVersion({ getVersion: async () => 'reppo 2024.01.05 v0.12.3', warn: () => {} })).toBe(true)
