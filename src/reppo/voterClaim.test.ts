...
/** Fake JSON-RPC fetch. claimableEpochs (`${epoch}:${pod}`) → claimVoterEmissions does NOT
 *  revert (something due); otherwise it reverts (nothing due — the authoritative oracle).
 *  claimed → hasUserClaimedEmissions true. */
...
describe('queryVoterClaimableOnchain', () => {
  it('claims epochs where the claim does not revert (something due)', async () => {
    const f = fakeFetch({ current: 107n, claimableEpochs: new Set(['105:50']) })
  ...
  it('skips an epoch that is due but already claimed', async () => {
    const f = fakeFetch({ current: 107n, claimableEpochs: new Set(['105:50']), claimed: new Set(['105:50']) })
    expect(await queryVoterClaimableOnchain('rpc', W, ['50'], undefined, { fetchImpl: f })).toEqual([])
  })

  it('claims nothing when every epoch reverts (nothing due)', async () => {
    const f = fakeFetch({ current: 107n, claimableEpochs: new Set() })
    expect(await queryVoterClaimableOnchain('rpc', W, ['50'], undefined, { fetchImpl: f })).toEqual([])
  })
  ...
  it('persists a watermark and does not re-scan resolved epochs next run', async () => {
    ...
    const f1 = fakeFetch({ current: 107n, claimableEpochs: new Set() }) // nothing due
    expect(await queryVoterClaimableOnchain('rpc', W, ['50'], cache, { fetchImpl: f1 })).toEqual([])
    expect(store['50']).toBe(106) // scanned through last closed epoch
    const f2 = fakeFetch({ current: 107n, claimableEpochs: new Set() })
  ...
  it('does NOT advance the watermark past a still-claimable (unclaimed) epoch', async () => {
    const store: Record<string, number> = {}
    const cache: VoterScanCache = { getThrough: (p) => store[p] ?? 0, setThrough: (p, e) => { store[p] = e } }
    const f = fakeFetch({ current: 107n, claimableEpochs: new Set(['103:50']) })
  ...
  it('returns [] when there are no closed epochs yet', async () => {
    const f = fakeFetch({ current: 1n, claimableEpochs: new Set() })
    expect(await queryVoterClaimableOnchain('rpc', W, ['50'], undefined, { fetchImpl: f })).toEqual([])
  })
})
