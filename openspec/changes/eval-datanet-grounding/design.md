...
```
`datanet.ts` holds the interface + `InMemoryDatanetSource`; `datanetClient.ts` binds it to `GET {base}/datanets` and `GET {base}/datanets/{id}/pods?limit=` with `Authorization: Bearer <REPPO_API_KEY>`, base = `EVAL_DATANET_API_URL ?? platformBase()`. Shape is an assumption mirroring eval-api's `client.ts`; both files change together when docs arrive. Failures throw (→ `:fail`), never read as "no evidence".
...
Per job: pods from every accessible datanet (cap `podsPerDatanet` = 200, cached per node for `cacheMs` = 5 min since datanets change slowly), `topKRelevant` over the union with k = 12. `RankedPod` gains `datanetId`. Lexical stays because several providers have no embeddings endpoint (existing amendment).
