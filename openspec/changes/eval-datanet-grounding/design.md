...
```
`datanet.ts` holds the interface + `InMemoryDatanetSource` + `DatanetError` (status-carrying, so 401/403 gets a named cause and a 10x backoff) + the TTL cache's `invalidate()` (called on a gateway `422 UNRESOLVABLE_CITATION`, so a pod deleted after caching is not re-cited by every job in the TTL window); `datanetClient.ts` binds it to `GET {base}/datanets` and `GET {base}/datanets/{id}/pods?limit=` with `Authorization: Bearer <REPPO_API_KEY>`, base = `EVAL_DATANET_API_URL ?? platformBase()`. Shape is PROVISIONAL — the datanet API is undocumented (see Open Questions). eval-api's `src/datanet/client.ts` is an INDEPENDENT guess at the same undocumented API, not a mirror of this one: the node needs LIST endpoints (`{base}/datanets`, `{base}/datanets/{id}/pods?limit=`), the gateway needs an EXISTENCE endpoint for one pod (`{base}/api/v1/datanets/{id}/pods/{podId}`), and the bases are separate env vars on separate deployments (`EVAL_DATANET_API_URL` here, `DATANET_API_URL` there) — the node's default `platformBase()` already ends in `/api/v1`, so the two values must never be copied across. Both files change together when docs arrive. A total read failure throws (→ `:fail`) and a partial one is reported as `unreadable` (D2); neither ever reads as "no evidence".
