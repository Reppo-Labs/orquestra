...
export function HealthTab({ health, loaded, datanets, activity, netNames }: {
   ...
   *  to reconcile them. Both now come from coverage(). */
  ...
  const cov = coverage(datanets, health.datanets, recoveredDatanets(activity))
  ...
  return (
    <div key="health">
      ...
      <div className="health-headline" role="status" aria-label="Datanet coverage">
        ...
        {cov.off > 0 && <span className="health-idle"> · {fmtCount(cov.off)} switched off</span>}
        {cov.blocked > 0 && <span className="health-idle"> · {fmtCount(cov.blocked)} need you</span>}
      </div>
