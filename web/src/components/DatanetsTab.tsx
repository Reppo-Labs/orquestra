...
function AdvancedPanel({ id, d, edit, providers }: {
  ...
  const setModel = (provider: string, model: string) =>
    upd((n) => {
      ...
      else n.model = { provider: provider as NonNullable<DatanetEntry['model']>['provider'], model }
    })
...
export function DatanetsTab({ strategy, netNames, health, datanetPnl, snapshot, pnl = null, activity = [], capsSignal, focusDatanet, onFocusConsumed, onReconfigure }: {
  strategy: Strategy
   ...
   *  Until they have, no datanet can honestly be called a loss-maker. Optional: absent
   *  reads as "no emissions yet", the conservative answer. */
  pnl?: Pnl | null
