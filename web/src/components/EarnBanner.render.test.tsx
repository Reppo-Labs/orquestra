...
const LOSING = pnl({ claimedReppo: 8383, earnedReppo: 8383, spentReppo: 10289.481, netReppo: -1906.481 })
...
describe('earnState', () => {
  ...
  it('calls a positive net a profit', () => {
    const s = earnState(pnl({ claimedReppo: 500, earnedReppo: 500, spentReppo: 200, netReppo: 300 }), earn())
    expect(s.tone).toBe('profit')
    expect(s.headline).toBe('Up 300 REPPO')
  })

  it('treats break-even as neutral, not as a gain', () => {
    const s = earnState(pnl({ claimedReppo: 100, earnedReppo: 100, spentReppo: 100, netReppo: 0 }), earn())
...
describe('<EarnBanner /> render', () => {
  ...
  it('uses the profit colour only when net REPPO is positive', () => {
    const { container } = render(
      <EarnBanner pnl={pnl({ claimedReppo: 500, earnedReppo: 500, spentReppo: 200, netReppo: 300 })} earn={earn()} />,
    )
