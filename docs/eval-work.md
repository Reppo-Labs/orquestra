...
Two compatibility windows close at different times, and each side owns one. The worker keeps serving criteria-bearing leases until the criteria-free gateway deployment has outlived one `SETTLE_DEADLINE_SECONDS` window (24 hours by default) — that covers a gateway rollback. The gateway keeps *sending* criteria-bearing leases, and keeps accepting `verdicts[]` completions, until the fleet has actually adopted this worker: nodes are self-hosted, so no one can force an operator to pull a new image. That window is weeks, it is measured by fleet adoption rather than by a deadline, and it is not the worker's to close. Existing shared `test/fixtures/lease-ack/` fixtures remain the legacy compatibility contract; new worker tests cover criteria-free requests and completions without changing those pinned fixtures unilaterally. `CRITERIA_MISMATCH` remains relevant only to legacy gateway responses; the worker treats any deterministic 422 as terminal.
