...
The two compatibility windows are nested, not parallel. The gateway keeps *sending* criteria-bearing leases, and keeps accepting `verdicts[]` completions, until the self-hosted fleet has actually adopted a criteria-free-capable worker — nobody can force an operator to pull a new image, so that window is weeks and is measured by adoption, not by a deadline. The worker's window contains the gateway's: it keeps serving criteria-bearing leases until the gateway has stopped sending them, and never drops the path on a clock of its own. After the gateway's cutover the worker must still serve them for a tail of at least one settlement-deadline window, covering in-flight jobs and a rollback; that deadline is a gateway-side setting, not a node knob, and nothing in this repo reads it. Existing shared `test/fixtures/lease-ack/` fixtures remain the legacy compatibility contract; new worker tests cover criteria-free requests and completions without changing those pinned fixtures unilaterally. `CRITERIA_MISMATCH` remains relevant only to legacy gateway responses; the worker treats any deterministic 422 as terminal.
